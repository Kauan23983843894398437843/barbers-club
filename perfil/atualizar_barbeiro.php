<?php
/**
 * Processamento de atualização de barbeiro
 */
...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    ...
    if (!$id_barbeiro) {
        $_SESSION['barbeiros_mensagem'] = 'ID do barbeiro inválido.';
    ...
    // Se não houver erros, tenta atualizar o barbeiro
    if (empty($erros)) {
        ...
        if (empty($erros)) {
            // Tenta atualizar o barbeiro
            $sucesso = $perfil->atualizarBarbeiro($id_barbeiro, $id_barbearia, $dados);
            
            if ($sucesso) {
                $_SESSION['barbeiros_mensagem'] = 'Barbeiro atualizado com sucesso!';
                $_SESSION['barbeiros_status'] = 'success';
            } else {
                $_SESSION['barbeiros_mensagem'] = 'Erro ao atualizar o barbeiro.';
                $_SESSION['barbeiros_status'] = 'error';
            }
        } else {
